...
public class CommitInspector {
	...
	public ArrayList<HashMap<String, String>> getNewCommits(Repository repo,
		...
		for (HashMap<String, String> item : comitsHistory) {
			...
			if (!isRowExists(name, date, message)) {
				long requestCode = insertRow(repoId, repo.getRepositoryName(),
						name, date, message, repo.getType());
	...
	public ArrayList<HashMap<String, String>> getCommitsHistory(Repository repo) {
		String repoId = repo.getId().toString();
		...
		int columnNameId = cursor
				.getColumnIndex(DatabaseOpenHelper.NAME_COLUMN);
		int columnDateId = cursor
				.getColumnIndex(DatabaseOpenHelper.DATE_COLUMN);
		int columnMsgId = cursor
				.getColumnIndex(DatabaseOpenHelper.MESSAGE_COLUMN);
		...
		while (cursor.moveToNext()) {
			...
			commitInfo.put(Repository.MESSAGE_TAG,
					cursor.getString(columnMsgId));
	...
	private boolean isRowExists(String name, String date, String message) {
		String[] columns = new String[] {
				DatabaseOpenHelper.REPOSITORY_ID_COLUMN,
				DatabaseOpenHelper.NAME_COLUMN, DatabaseOpenHelper.DATE_COLUMN,
				DatabaseOpenHelper.MESSAGE_COLUMN };
		String whereClause = DatabaseOpenHelper.NAME_COLUMN + "=? AND "
				+ DatabaseOpenHelper.DATE_COLUMN + "=? AND "
				+ DatabaseOpenHelper.MESSAGE_COLUMN + "=?";
		String[] whereArgs = new String[] { name, date, message };
		Cursor cursor = DB.query(DatabaseOpenHelper.TABLE_NAME, columns,
				whereClause, whereArgs, null, null, null);
	...
	private long insertRow(String repoId, String repoName, String name,
			String date, String message, String vcs) {
